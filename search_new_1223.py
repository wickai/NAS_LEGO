...
class SepConv(nn.Module):
    ...
    def __init__(self, inp, oup, k, s):
        ...
        self.conv = nn.Sequential(
            # Depthwise
            nn.Conv2d(inp, inp, k, s, k // 2, groups=inp, bias=False),
            nn.BatchNorm2d(inp),
            nn.ReLU6(inplace=True),
            # Pointwise
...
class DilConv(nn.Module):
    """空洞可分离卷积"""
    def __init__(self, inp, oup, k, s, dilation):
        super().__init__()
        self.use_res = (s == 1 and inp == oup)
        pad = (k // 2) * dilation
        self.conv = nn.Sequential(
            # Dilated Depthwise
            nn.Conv2d(inp, inp, k, s, pad, dilation=dilation, groups=inp, bias=False),
            nn.BatchNorm2d(inp),
            nn.ReLU6(inplace=True),
            # Pointwise
            nn.Conv2d(inp, oup, 1, bias=False),
...
class ConvBNReLU(nn.Module):
    ...
    def __init__(self, inp, oup, k, s):
        ...
        self.use_res = (s == 1 and inp == oup)
        self.conv = nn.Sequential(
            nn.Conv2d(inp, oup, k, s, k // 2, bias=False),
            nn.BatchNorm2d(oup),
            nn.ReLU6(inplace=True),
        )
...
class PoolBN(nn.Module):
    """池化 + 可选投影"""
    def __init__(self, inp, oup, s, pool_type='avg'):
        super().__init__()
        self.use_res = (s == 1 and inp == oup)
        if pool_type == 'avg':
            self.pool = nn.AvgPool2d(3, stride=s, padding=1, count_include_pad=False)
        else:
            self.pool = nn.MaxPool2d(3, stride=s, padding=1)
        # 通道投影
        self.proj = None
        if inp != oup:
...
class MobileNetV2(nn.Module):
    ...
    def __init__(self, op_codes, width_codes, stage_setting, op_list, width_choices,
        ...
        self.op_list = list(op_list)  # 保存一下以便外部引用（可选）
    ...
    def _build_ops(self, op_list):
        ...
        # MBConv 变体：卷积核 3/5/7，扩展率 1/2/3/4/6，有/无SE
        for k in (3, 5, 7):
            for r in (1, 2, 3, 4, 6):
        ...
        # 可分离卷积：卷积核 3/5/7
        for k in (3, 5, 7):
            self._ops[f"sep_conv_{k}x{k}"] = lambda i, o, s, t, k=k: SepConv(i, o, k, s)

        # 空洞卷积：卷积核 3/5，空洞率 2/4
        for k in (3, 5):
            for d in (2, 4):
                self._ops[f"dil_conv_{k}x{k}_d{d}"] = lambda i, o, s, t, k=k, d=d: DilConv(i, o, k, s, d)

        # 标准卷积：卷积核 1/3/5
        for k in (1, 3, 5):
            self._ops[f"conv_{k}x{k}"] = lambda i, o, s, t, k=k: ConvBNReLU(i, o, k, s)

        # 池化操作
        self._ops["avg_pool_3x3"] = lambda i, o, s, t: PoolBN(i, o, s, 'avg')
        self._ops["max_pool_3x3"] = lambda i, o, s, t: PoolBN(i, o, s, 'max')

        # 投影残差（不匹配时 1x1 Conv-BN）
...
class MobileNetSearchSpace:
    ...
    # 扩展宽度选择：5种
    _WIDTH_CHOICES = [0.35, 0.5, 0.75, 1.0, 1.25]

    @staticmethod
    def _default_op_list():
        """
        扩展后的算子列表：
        - MBConv: 3种卷积核(3/5/7) × 5种扩展率(1/2/3/4/6) × 2(有/无SE) = 30种
        - SepConv: 3种卷积核(3/5/7) = 3种
        - DilConv: 2种卷积核(3/5) × 2种空洞率(2/4) = 4种
        - Conv: 3种卷积核(1/3/5) = 3种
        - Pool: avg_pool + max_pool = 2种
        - skip_connect + zero = 2种
        总计: 30 + 3 + 4 + 3 + 2 + 2 = 44种算子
        """
        ops = []
        ...
        # 可分离卷积：3种
        for k in (3, 5, 7):
            ops.append(f"sep_conv_{k}x{k}")
        
        # 空洞卷积：4种
        for k in (3, 5):
            for d in (2, 4):
                ops.append(f"dil_conv_{k}x{k}_d{d}")
        
        # 标准卷积：3种
        for k in (1, 3, 5):
            ops.append(f"conv_{k}x{k}")
        
        # 池化操作：2种
        ops.append("avg_pool_3x3")
        ops.append("max_pool_3x3")
...
class EvolutionarySearch:
    def __init__(self, population_size, mutation_rate, n_generations,
                 ...
                 num_inits=1, eval_loader=None, n_blocks_to_search=None):
        ...
        # 支持搜索指定数量的 blocks
        self.n_blocks = n_blocks_to_search if n_blocks_to_search else search_space.total_blocks
        logging.info(f"[Global EA] Searching {self.n_blocks} blocks (total: {search_space.total_blocks})")

    def _random_op_codes(self):
        """生成指定长度的随机 op_codes"""
        return [random.randrange(len(self.search_space.op_list)) for _ in range(self.n_blocks)]

    def _mutate_op_codes(self, codes):
        """变异 op_codes"""
        mutated = codes[:]
        for i in range(len(mutated)):
    ...
    def search(self, inputs):
        ...
        for _ in range(self.population_size):
            pop.append({
                "op_codes": self._random_op_codes(),  # 使用指定长度
        ...
        for g in range(self.n_generations):
            ...
            while len(next_gen) < self.population_size:
                ...
                child_ops = self._mutate_op_codes(child_ops)  # 使用内部变异
        ...
        return {
            "op_codes_prefix": best["op_codes"],  # 前缀 op_codes
...
def parse_args():
    p = argparse.ArgumentParser("MobileNetV2 Search with SWAP: layer-wise EA or global EA; then full training")
    p.add_argument("--log_path", default="./logs", type=str)
    p.add_argument("--log_name", default="layerwise_es.log", type=str)
...
def main():
    ...
    if args.search_mode == "layer_ea":
        ...
        es = EvolutionarySearch(
            ...
            n_blocks_to_search=args.n_blocks_to_search  # 现在支持！
        )
        best = es.search(mini_inputs)
    t1 = time.time()

    logging.info(f"Search finished in {t1 - t0:.2f}s.")
    logging.info(f"Best architecture | SWAP(prefix)={best['fitness']:.3f}")
    ...
    logging.info(f"Best Prefix Model param: {best.get('params_mb_prefix', 0):.2f} MB")

    # 构造最优“前缀模型” & 训练（不补齐剩余 blocks）
    model = sp.get_prefix_model(best["op_codes_prefix"], best["width_codes"])
