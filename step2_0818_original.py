...
# -*- coding: utf-8 -*-
"""
基于SWAP分数的逐步网络构建实验（强化版 / Two-Stage, Multi-Objective, Cached Features）
...
6) 其余接口保持不变（BaseSwap1/mbconv_block/prepare_batches/set_seed）。
"""
...
def generate_candidate_specs(
    ...
    for _ in range(n):
        ...
        stride = random.choice([1, 2])  # 可按需要收缩/放宽
...
def run_experiment(args):
    ...
    batches = prepare_batches(args.data_root, max(args.batches_fast, args.batches_final), args.batch_size, device)

    net = StackedNet(num_classes=args.num_classes).to(device)
    history: List[Dict[str, Any]] = []
    prev_chosen_spec: Dict[str, Any] | None = None

    for step in range(args.max_steps):
        print(f"\n步骤 {step + 1}/{args.max_steps} | 当前通道 = {net.current_channels}")
        ...
        for spec in tqdm(candidate_specs, desc="Fast评估候选块"):
            ...
            flops_g, params_m = try_profile_flops_params(block, cached_feats_fast[0]) if len(cached_feats_fast) > 0 else (0.0, sum(p.numel() for p in block.parameters())/1e6)
        ...
        print(f"步骤 {step + 1} | 最佳综合分: {best_score:.5f} | 规格: {best_spec} | 指标: {best_metrics}")
...
# -----------------------------
def build_parser():
    p = argparse.ArgumentParser("基于Block SWAP的逐步网络构建实验（强化版）")
    # 搜索规模
    ...
    # 数据
    p.add_argument("--batches", type=int, default=3, help="prepare_batches 的最大 batch 数（与 batches_final 保持一致或更大）")
